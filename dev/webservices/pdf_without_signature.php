...
class PdfSignature extends DB {
	...
	if(!empty($_REQUEST['name']) && !empty($_REQUEST['email']) && !empty($_REQUEST['id']) && !empty($_REQUEST['document_id']) && !empty($_REQUEST['document_number_id']) && !empty($_REQUEST['address']) && !empty($_REQUEST['amount']) && !empty($_REQUEST['no_of_months']) ) { 
		...
		$html = '<!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 4.0 Transitional//EN">
			...
			$mpdf->WriteHTML($html);
			// $mpdf->Output();
			$location = __DIR__ .'/pdf_without_signature/';
			$pdf_file = 'user_pdf'.uniqid().time().'.pdf';
			$mpdf->Output($location . $pdf_file, \Mpdf\Output\Destination::FILE);

			$pdf_without_signature = PDF_WITHOUT_SIGNATURE.$pdf_file;


            $response = array('status' => 'true', 'message' => 'Upload successfully!', 'file_path' => $pdf_without_signature);
