...
// JSON API //
...

			{
			   ...
			   "BusinessParameters": {
				  "CreditProviderCode": "RGNT0025",
				  "CreditProviderConsentToCCRCWAEnquiry": "1",
			...
			$cofaceResponse = curl_exec($curl);  var_dump($cofaceResponse); die;
 ...
 echo $poststring = htmlentities($request);
...
$curl = curl_init();
			...
			curl_setopt_array($curl, array(
			  CURLOPT_URL => 'https://stgpercert.bdi.co.il/Front/FrontService.svc',
			  CURLOPT_RETURNTRANSFER => true,
			  CURLOPT_ENCODING => "",
			  CURLOPT_MAXREDIRS => 10,
			  CURLOPT_TIMEOUT => 0,
			  ...
			  CURLOPT_HTTPHEADER => array("Content-Type: text/xml; charset=utf-8", "Content-length: ".strlen($poststring)),
			));


             
                  $data1['bdireport'] = curl_exec($curl);
                  
                  
                  var_dump($data1['bdireport']); die;


 $_SERVER["DOCUMENT_ROOT"].'/dev/bdijson.txt';

			   $data1['bdireport'] = file_get_contents($_SERVER["DOCUMENT_ROOT"].'/dev/bdijson.txt');
			  
			  $full = json_decode($data1['bdireport'],true);
			  
			 echo '<pre>'; echo $full['BDiInformation']['AggregatedActiveDataDebtorCCR']['AggregatedCurrentAccountDebtors'][0]['StartingDate'];
			 
			 $StartingDate = preg_replace( '/[^0-9]/', '',$full['BDiInformation']['AggregatedActiveDataDebtorCCR']['AggregatedCurrentAccountDebtors'][0]['StartingDate']);
			 
			echo 'after conver date is ' . $StartingDate = date("Y-m-d H:i:s", $StartingDate / 1000);
			 
			 $full['BDiInformation']['AggregatedActiveDataDebtorCCR']['AggregatedCurrentAccountDebtors'][0]['PlannedEndDate'] = $StartingDate;
         die;
    }
...
<ApplicationContractType>MORTGAGE</ApplicationContractType>

<CreditAmount>10000</CreditAmount>

<CreditProviderApplicationCode/>

<Designation>6</Designation>

<PaymentFrequencyInAYear/>

<PlannedMonthlyPayment>0</PlannedMonthlyPayment>
...
<CreditProviderConsentToCCRCWAEnquiry>1</CreditProviderConsentToCCRCWAEnquiry>

<FlagCustomerConsent>1</FlagCustomerConsent>

<FlagCustomerIdentificationWhenCustomerGaveConsentAccordingToRegulations>1</FlagCustomerIdentificationWhenCustomerGaveConsentAccordingToRegulations>

<FlagCustomerInformed>1</FlagCustomerInformed>

<PWS>D6PBCTKB</PWS>

<RecommendationModelId>0</RecommendationModelId>

<TermId>513578674</TermId>

<UnregisteredCreditProviderDescription>0</UnregisteredCreditProviderDescription>
...
<Reqtype>NAEWS10</Reqtype>


<Subject>

<ACTIVITY_VOLUME_LAST_YEAR>0</ACTIVITY_VOLUME_LAST_YEAR>
...
<EXPERIENCE_WITH_CLIENT>0</EXPERIENCE_WITH_CLIENT>
...
//                            die();
                            ...
                            if(!empty($private1)){
                                ...
                                elseif($private1['radio-box'] == "מסלול ללא פיקדון"){
                                    // redirect('private32');
                                    $this->session->set_userdata('order_id_number', @$clientData->client_reponse_data[1]->order_id_number);
                                    echo "<script type='text/javascript'>window.location.href = '".site_url('private6')."';</script>";
                                    return FALSE;
                                }
                                else{
                                    // redirect('private31');
...
//            print_r($data);
            ...
            $fields = array("amt_company_name__c" => $company_name,
            ...
            "amt_company_contact_person_phone__c" => $contact_person_phone,'order_id_number__c' => $order_id_number,'flow_number__c'=>1,'client_id_number'=>$unique_id,
...
//            print_r($data);
            ...
            $prevousnfo = $this->session->userdata('private3');
            
                    $another_account_address = $data['apartment_add'];
                    $another_first_name = $data['landlord'];
                    $another_account_id_number = $data['landlord_id'];
                    $another_account_phone = $data['property_phone'];
                    $another_account_phone = str_replace(["-"], '', $another_account_phone);
                    $another_account_email = $data['sec_email'];
                    $ant_account_file = $data['another_user_file'];
            
             $fields = array( "another_compnay_first_name__c" => $another_first_name, "another_compnay_id__c" => $another_account_id_number,
             "another_compnay_email__c" => $another_account_email,
             "another_compnay_phone__c" => $another_account_phone, "another_compnay_address__c" => $another_account_address,
             "another_compnay_file__c" => $data['another_user_file'], 'order_id_number__c' => $order_id_number,'client_id_number'=>$unique_id,'ref_client_id__c'=>$unique_id);
...
//            print_r($data);
        ...
        // second signature image start
            define('UPLOAD_SECOND_DIR', 'doc_sign/');
              ...
              $file_path21 = $_SERVER["DOCUMENT_ROOT"].'/'.$file2;
              // $file_path2 = "123";
...
צורך בדיקת הבקשה, אנחנו מתכוונים לפנות ללשכת אשראי על מנת לקבל חיווי אשראי. מובהר בזאת שלצורך קבלת החיווי לשכת האשראי תגיש בקשה לבנק ישראל לקבלת נתוני אשראי לגביך הכלולים במאגר. הפנייה ללשכת אשראי לצורך קבלת חיווי הינה בכל מקרה של בקשה לקבלת הלוואה באמצעות טריא. </SPAN></FONT></P>
          ...
          <P DIR="RTL" CLASS="western text_color" STYLE="margin-left: 0.02in; margin-bottom: 0in; line-height: 100%; text-align:center; ">
            ...
            <img src="'.$file_path21.'" class="img-responsive" style="margin-left: 20px;">
...
//            $diff_data = array('diff' => $diff, 'an_diff' => $an_diff, 'sec_diff' => $sec_diff, 'client_file' => $client_first_file, 'another_account_file' => $another_account_file, 'order_file_name' => $order_file_name, 'file_path1' => $file_path1, 'file_path2' => $file_path2 );
    ...
    $clientID = $unique_id; $signaturepath = $_SERVER["DOCUMENT_ROOT"].'/website_assets/img/sign1.png';
...
כל דרישה על פי ערבות זו צריכה להתקבל על ידינו בכתב ולא יאוחר מהתאריך הנ"ל
        ...
        $this->session->set_userdata('url_user_document',$pdf_file);
           ...
           $fields = array( 'order_id_number__c' => $order_id_number,'flow_number__c'=>1,'client_id_number'=>$unique_id,'Client__c'=>$prevousnfo['client_c'],'document_number__c'=>$document_id,'ref_client_id__c'=>$unique_id);
...
//               
                             ...
                             $this->session->set_userdata('flow_number__c','1');
...
//            redirect('private8');
            ...
            $this->load->view('website/header');
            $this->load->view('website/private/private_7_scr',$data1);
            $this->load->view('website/footer', $data);
        }
...
//            print_r($this->session->userdata()); die();
        ...
        $private3 = $this->session->userdata('private3');
            ...
            $cofaceUrl = 'https://personalcheckcert.bdi.co.il/FrontRest/FrontServiceRest.svc/PostRequest';
            
            
            echo $requst_json = '{"Reference":"Ref123456","Reqtype":"NAEWS10","Origin":"","Subject":{"IdType":1,"Id":'.$unique_id.',"CCode":"","FirstName":"לירן","LastName":"חולצה ","LatinFirstName":"","LatinLastName":"","BankId":0,"BranchId":0,"AccountNum":0,"EXPERIENCE_WITH_CLIENT":2,"PAYMENTS_ETHICS":3,"ACTIVITY_VOLUME_LAST_YEAR":1},"Application":{"ApplicationContractType":"MORTGAGE","Role":"1","CreditAmount":10000,"PlannedStartingDate":"2018-09-10","PaymentFrequencyInAYear":"10","PlannedMonthlyPayment":0,"CreditProviderApplicationCode":"","Designation":"6"},"BusinessParameters":{"User":"WS_GOLD_1212","PWS":"D6PBCTKB","TermId":"513578674","CreditProviderCode":"RGNT0025","FlagCustomerInformed":"1","UnregisteredCreditProviderDescription":"","FlagCustomerConsent":"1","FlagCustomerIdentificationWhenCustomerGaveConsentAccordingToRegulations":"1","CreditProviderConsentToCCRCWAEnquiry":0}
...
echo PHP_EOL;
			...
			curl_setopt_array($curl, array(
			  ...
			  CURLOPT_SSL_VERIFYHOST=> 0,
			  CURLOPT_CUSTOMREQUEST => "POST",
			  CURLOPT_POSTFIELDS =>$requst_json,
			  CURLOPT_HTTPHEADER => array(
				"Content-Type: application/json"
			  ),
			));

			  echo $data1['bdireport'] = curl_exec($curl);
            ...
            $req_gur_amt = str_replace(',','', $private3["private2_requested_gurantee_amt"]);
            ...
            $company_address = $private5['company_address'];
            
            $Area__c = $private5['Area__c'];
            $Business_Field__c = $private5['Business_Field__c'];
        
        $full =$data1['bdireport']; $approve_status = 2;
        ...
        $request = array('TestCall'=>true,'ClientInternalCallID'=>'7086','ClientInternalCustomerID'=>$unique_id,'PersonalID'=>'108904329',
        'GuaranteeType'=>1,'GuaranteeAmount'=>(int)$req_gur_amt,'GuaranteePeriod'=>18,'TimeInBusiness'=>4,'ExperienceInBusiness'=>12,'BusinessArea'=>2,
        'DateOfBirth'=>$client_date_of_birth,'GeographicalArea'=>5,'BDIBusinessReport'=>json_decode($full));
        
        echo json_encode($request); echo PHP_EOL.PHP_EOL.PHP_EOL;
        
        
        
        $request_json = '{"TestCall":true,"ClientInternalCallID":"7086","ClientInternalCustomerID":'.(int)$unique_id.',"PersonalID":'.$unique_id.',"GuaranteeType":1,"GuaranteeAmount":'.$req_gur_amt.',"GuaranteePeriod":18,"TimeInBusiness":4,"ExperienceInBusiness":12,"BusinessArea":'.$Business_Field__c.',"DateOfBirth":'.$client_date_of_birth.',"GeographicalArea":'.$Area__c.',"BDIBusinessReport":'.$full.'}';
        
        
       // echo $request_json = json_encode($request, JSON_UNESCAPED_UNICODE); 
			...
			curl_setopt_array($curl, array(
			 ...
			 echo $cofaceResponse = curl_exec($curl); die;
...
//            print_r($this->session->userdata());die();
	...
	function private_payment_api() {
        ...
        // second signature image start
            define('UPLOAD_SECOND_DIR', 'doc_sign/');
              $img2 = $postForm['second_base64_signature'];
              ...
              $file2 = UPLOAD_SECOND_DIR . uniqid().rand() . '.png';
              // echo $file; die();
              $success2 = file_put_contents($file2, $data2);
              ...
              $new_doc_sign = $postForm['first_base64_signature'];
            ...
            $req_gur_amt = str_replace(',', '',$_POST["private2_requested_gurantee_amt"]);
            ...
            $business_type = $postForm['business_type'];
            ...
            $client_id = $private2['unique_id'];
            
            $clientAdd = $hometown.','.$street.','.$home_no;
            
             $expdate = $postForm['expiry_date'];
            
            $expdate2 = $postForm['expiry_date2'];
...
//             die();
            ...
            if(!empty($err)){
             ...
             $data['client_c'] = $prevousnfo['client_c'];
            
             $this->createandSaveAllDocuments($postForm);
            
             echo json_encode(array('status'=>true,'msg'=>'selesforce inserted true','data'=>$fields));
...
//        print_r($_REQUEST);
    ...
    function createandSaveAllDocuments($data){
        ...
        /*    <div style="text-align: center;border-bottom: 1px solid black; padding: 10px; margin-top: -150px;">
           ...
           <P DIR="RTL" CLASS="western text_color" STYLE="margin-left: 0.02in; margin-bottom: 0in; line-height: 100%">
            ...
            <FONT FACE="David"><SPAN LANG="he-IL"> 2.3. במקרה של מימוש כתב הערבות ע"י המוטב, יוכל החייב, על פי שיקול דעתו, להשתמש בהפניית הערב  </SPAN></FONT></P>
             ...
             <P DIR="RTL" CLASS="western text_color" STYLE="margin-left: 0.02in; margin-bottom: 0in; line-height: 100%">
            <FONT FACE="David"><SPAN LANG="he-IL"> לקבלת הלוואה דרך חברת טריא בריבית שנתית קבועה של 6.5% ההלוואה תעמוד לתקופה  </SPAN></FONT></P>
...
*/
        ...
        //$html2 .= $data['documentcontent'];

         $html2 .='   <DIV CLASS="" style="text-align: center; ">
     ...
     <!-- <table STYLE="width:100%;">
        <tr>
            ...
            <OL>
                ...
                <LI><P DIR="RTL"><FONT FACE="David"><SPAN LANG="he-IL"> אנו ערבים בזה כלפיך/ם לתשלום כל סכום עד לסכום כולל של <B><U>'.$amount.'</U></B> ₪ (להלן: "סכום הערבות") שתדרוש/תדרשו מאת <B><U>'.$name.'</B></U>  ת.ז <B><U>'.$clientID.'</U></B> (להלן וביחד: "הנערב") בקשר עם ההסכם מיום <B><U>'.$startDate.'</U></B>, על כל תוספותיו, ככל שיהיו מעת לעת (להלן: <B>"ההסכם"</B>).
 </SPAN></FONT></P>
...
כל דרישה על פי ערבות זו צריכה להתקבל על ידינו בכתב ולא יאוחר מהתאריך הנ"ל
        ...
        $this->session->set_userdata('url_user_document',$pdf_file);
           ...
           "guarantee_document__c"=>$house_rent_document,
            'order_id_number__c' => $order_id_number,'flow_number__c'=>1,'client_id_number'=>$unique_id,'Client__c'=>$prevousnfo['client_c'],'document_number__c'=>$document_id,'ref_client_id__c'=>$unique_id);
    ...
    function createdocument(){
            ...
            $data = $this->session->userdata('private3');
            $private1 = $this->session->userdata('private1');
            $preferred_route = $private1['radio-box'];
...
$htmlsdata = base64_decode($_POST['datatext']);
        ...
        // end second
           ...
           $fields = array( "second_document_url__c" => $second_document,
            'order_id_number__c' => $order_id_number,'flow_number__c'=>1,'client_id_number'=>$unique_id,'Client__c'=>$prevousnfo['client_c'],'ref_client_id__c'=>$unique_id);
...
function callparetix(){
        ...
        $businessThird3 = $this->session->userdata('private5');
        ...
        $request_json = '{"TestCall":true,"ClientInternalCallID":"7086","ClientInternalCustomerID":'.$unique_id.',"PersonalID":'.$unique_id.',"GuaranteeType":1,"GuaranteeAmount":'.$req_gur_amt.',"GuaranteePeriod":18,"TimeInBusiness":4,"ExperienceInBusiness":12,"BusinessArea":'.$Business_Field__c.',"DateOfBirth":'.$client_date_of_birth.',"GeographicalArea":'.$Area__c.'}';
