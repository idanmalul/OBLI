...
class FullyVerified extends CI_Controller {
    ...
    public function kyc(){
        ...
        if(!empty($this->session->userdata('flow_number__c'))){
            
            if($this->session->userdata('flow_number__c') == 5){
                
                $sessiion_data = $user_data['businessThird1'];
                
            }
            
            if($this->session->userdata('flow_number__c') == 3){
                
                $sessiion_data = $user_data['business3'];
                
            }
            
            if($this->session->userdata('flow_number__c') != 5 && $this->session->userdata('flow_number__c') != 3){
                 $sessiion_data = $user_data['private2'];
            }
        ...
        if(
            isset($sessiion_data) &&
            $sessiion_data['first_name']!=null &&
            $sessiion_data['last_name']!=null &&
            $sessiion_data['unique_id']!=null
        ){
            $insert_data = array(
                'last_name' => $sessiion_data['last_name'],
                'first_name' => $sessiion_data['first_name'],
                'id_number' => $sessiion_data['unique_id'],
                'status' => 'pending',
                'verified' => 0,
                'send_data_sf' => 0,
            );
            $verified_id = $this->project_model->insert_data('verified_persons', $insert_data);
            $result = $this->create_verification($verified_id, $sessiion_data['client_email']);
            ...
            echo "<script type='text/javascript'>alert('Error!'); window.location.href = '".site_url($redirect)."'; </script>";
            return FALSE;
        }
    ...
    public function check_verified_status(){
        ...
        if(isset($id) && $id!=null){
            ...
            $msg = "First line of text\nSecond line of text id - ".$id;

            
            // send email
            mail("manikant.tectum@gmail.com","otp code",$msg);
